...
void ExpandNeighbors(vector<int> current, int goal[2], vector<vector<int>> &openlist, vector<vector<State>> &grid)
{
  ...
  for (int i = 0; i < 4; i++)
  {
    int neighbor_x = current_x + delta[i][0];
    //std::cout<< "Neighbor_x = " << neighbor_x <<"\n";
    int neighbor_y = current_y + delta[i][1];
    //std::cout<< "Neighbor_y = " << neighbor_y <<"\n";

    //std::cout << "Neighbor: " << neighbor_x << ", " << neighbor_y << "\n";
    
    if (CheckValidCell(neighbor_x, neighbor_y, grid))
    {
      std::cout << "Checking: " << neighbor_x << ", " << neighbor_y << "\n";
      int neighbor_g = current_g + 1;
      int neighbor_h = Heuristic(neighbor_x, neighbor_y, goal[0], goal[1]);
      AddToOpen(neighbor_x, neighbor_y, neighbor_g, neighbor_h, openlist, grid);
